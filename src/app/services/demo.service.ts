  ...
  async generateDemoData() {
    try {
      const clients: Client[] = [
        { name: 'Juan Pérez', alias: 'JP', phone: '123-456-7890', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { name: 'María Gómez', alias: 'MG', phone: '987-654-3210', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { name: 'Carlos Rodríguez', alias: 'CR', phone: '555-555-5555', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { name: 'Ana Torres', alias: 'AT', phone: '222-333-4444', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { name: 'Luis Martínez', alias: 'LM', phone: '777-888-9999', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() }
      ];
      ...
      const lotteries: Lottery[] = [
        { name: 'Lotería Nacional', type: 'triple', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { name: 'Animalitos', type: 'animal', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { name: 'Mega Chance', type: 'terminal', created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() }
      ];
      for (let i = 0; i < lotteries.length; i++) {
        ...
        const options = this.lotteryOptionService.generateOptions(createdLottery.type, createdLottery.id!).map(option => ({
          ...option,
      ...
      const multipliers: LotteryMultiplier[] = [
        { lottery_id: 1, type: 'triple', multiplier: 10, created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { lottery_id: 2, type: 'animal', multiplier: 5, created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() },
        { lottery_id: 3, type: 'terminal', multiplier: 8, created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() }
      ];
      ...
      for (const day of days) {
        ...
        for (let i = 0; i < drawsPerDay; i++) {
          schedules.push({ dayOfWeek: day, time: ['12:00', '15:00', '18:00', '21:00'][i], lotteryId: 1, isActive: true, created_at: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString() });
        }
      }
      ...
      for (let day = 0; day < 3; day++) {
        for (let ticketNum = 0; ticketNum < 20; ticketNum++) {
          const ticket: Ticket = {
            created_at: new Date(new Date().setDate(new Date().getDate() - day)).toISOString(),
